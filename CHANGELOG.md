...
- **Editable "My account" withdrawal endpoint slug.** The withdrawal endpoint under My account (previously the fixed `…/withdrawals/`) can now be renamed in **WooCommerce → Settings → Advanced → Account endpoints**, next to WooCommerce's own endpoint slugs (option `woocommerce_myaccount_withdrawals_endpoint`, default `withdrawals`) — handy on non-English stores (e.g. `/fiokom/elallasaim/`). Only the public URL slug changes; the internal routing key is unchanged, so renaming never breaks the endpoint, its account-menu link or the `woocommerce_account_withdrawals_endpoint` hook. WooCommerce persists and flushes it on save; a change made outside the settings screen (WP-CLI/programmatic) flushes on the next request.
...
- **Rewrite rules re-flush once after a plugin update, not only on activation.** Because a plugin update does not re-run the activation hook, any endpoint/rewrite change shipped in an update previously required a manual reactivation to take effect. Rules are now flushed once per plugin-version change (tracked via the `lw_elallas_version` option), at `init` priority 20 after the endpoint registers.
